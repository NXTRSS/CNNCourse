...
source ml_cv/venv/bin/activate

pip install -r requirements.txt

jupyter nbextension enable codefolding/edit
python -m ipykernel install --user --name ml_basic --display-name "Python (ml_cv)”
...
```bash
git clone https://github.com/NXTRSS/CNNCourse
```
...
```bash
jupyter nbextension enable codefolding/edit
python -m ipykernel install --user --name ml_cv --display-name "Python (ml_cv)”
```
...
**Ważne: w narzędziu Jupyter Notebook do uruchamiania przygotowanych skryptów trzeba będzie "wyklikać" kernel *Python (ml_cv)* - będzie to pokazane na zajęciach.**\
